...
void BoxTools::getProbaVectorFromSingleBox(const cv::Mat_<uint8_t>& unitBox, vector<double>& vect) const {
    vect.clear();
    for (int k{0}; k < box_size_; k++) {//img first
            for (int j{0}; j < box_size_; j++) {//then row
                for (int i{0}; i < box_size_; i++) {//then col
                vect.push_back(pix2proba(unitBox(i,j,k)));//this is (col, row, img)
            }
        }
    }
}
...
void BoxTools::getSingleBoxFromVector(cv::Mat_<uint8_t>& unitBox, const vector<double>& vect) const {
    for (int k{0}; k < box_size_; k++) {//img first
            for (int j{0}; j < box_size_; j++) {//then row
                for (int i{0}; i < box_size_; i++) {//then col
                unitBox(i,j,k) = getNormalizedPixValue(vect.at((k*box_size_*box_size_)+j*box_size_+i));//this is (col, row, img)
            }
        }
    }
}
...
void BoxTools::blurSingleBox(cv::Mat_<uint8_t>& refMat, double sigma, int ksize, double additionnal_uniform_noise) const {
    ...
    get3DProbaImageFromSingleCube(refMat, noisygt);
    noisygt -= 0.5;
    ...
    cv::Size kernelsize = cv::Size(ksize,ksize);
    ...
    getSingleBoxFromVector(refMat, vect);
